...
OnControlMsgCallback(void *pClient, const char *pJsonValueBuffer, uint32_t valueLength, DeviceProperty *pProperty) {
    ...
    for (i = 0; i < TOTAL_PROPERTY_COUNT; i++) {
        ...
        if (strcmp(sg_DataTemplate[i].data_property.key, pProperty->key) == 0) {
            ...
            // parse json data to local sg_DataTemplate
            char *temp = LITE_json_value_of(pProperty->key, pJsonValueBuffer);
            strcpy(sg_DataTemplate[i].data_property.data, temp);
...
//    Log_d("someting about your own product logic wait to be done");
    if (sg_DataTemplate[0].state == eCHANGED) {
        if (pData->m__mesh_generic_onoff == '1') {
...
static int _get_sys_info(void *handle, char *pJsonDoc, size_t sizeOfBuffer) {
    /*platform info has at least one of module_hard_info/module_soft_info/fw_ver property*/
    DeviceProperty plat_info[] = {
            {.key = "module_hard_info", .type = TYPE_TEMPLATE_STRING, .data = "ESP8266"},
            {.key = "module_soft_info", .type = TYPE_TEMPLATE_STRING, .data = "V1.0"},
    ...
    DeviceProperty self_info[] = {
            {.key = "append_info", .type = TYPE_TEMPLATE_STRING, .data = "unused"},
            {NULL, NULL, 0}  //end
    };
...
static int tc_data_template(int argc, char **argv) {
    rt_thread_t tid;
...
MSH_CMD_EXPORT(tc_data_template, startup data template);
#endif